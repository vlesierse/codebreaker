    ...
    enterCode: (code) => (dispatch, getState) => {
        ...
        if (game.id) {
            let fetchTask = fetch(`/api/game/${game.id}/code`, { headers: { 'Content-Type': 'application/json' }, method: "POST", body: JSON.stringify(code)})
                .then(response => response.json())
                .then(data => { dispatch({ type: actionTypes.RECEIVED_CODERESULT, attempt: data }); });
            addTask(fetchTask);
...
export const reducer = (state = initialState, action) => {
    switch (action.type) {
        case actionTypes.LOADED_GAME:
            return { ... state, id: action.game.id, attempts: action.game.attempts }
        case actionTypes.ENTERED_CODE:
            return { ... state, isWaiting: true };
        case actionTypes.RECEIVED_CODERESULT:
            return { ... state, isWaiting: false, attempts: [ ... state.attempts, action.attempt ]  };
    }
    return state;
};
